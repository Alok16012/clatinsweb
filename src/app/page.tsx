...
export default async function Home() {
  ...
  return (
    <main>
          ...
          <div className="md:hidden flex items-center justify-between px-4 mb-2">
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-bold uppercase tracking-wider" style={{ color: '#f77420' }}>{homeContent.courses.mobileEyebrow}</span>
              </div>
              <h2 className="font-black" style={{ color: '#0D1837', fontSize: '20px', lineHeight: 1.2 }}>{homeContent.courses.mobileTitle}</h2>
            </div>
            <Link href={homeContent.courses.seeAllLink} className="text-xs font-bold" style={{ color: '#f77420' }}>{homeContent.courses.seeAllLabel}</Link>
          </div>
      ...
      <CollegePredictorSection content={homeContent.predictor} />
      <Footer />
    </main>
  );
}
